...
describe("FriendsController", () => {
  ...
  describe("GET /friends/requests/:id", () => {
    beforeEach(async () => {
      user1.friendRequestsReceived.push(user2._id);
      await user1.save();
    });

    test("retrieves friend requests", async () => {
      const response = await request(app).get(`/friends/requests/${user1._id}`);
      expect(response.status).toBe(200);
      expect(response.body.friendRequestsReceived.length).toBe(1);
      expect(response.body.friendRequestsReceived[0]._id).toEqual(
        user2._id.toString()
      );
    });

    test("returns an error if retrieving friend request list fails", async () => {
      const response = await request(app).get(`/friends/requests/invalidId`);

      expect(response.status).toBe(400);
      expect(response.body.message).toBe(
        "Something went wrong with retrieving the friend request list"
      );
    });
  });
});
